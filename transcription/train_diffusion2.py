...
def valid_step(trainer, batch, ema, step, device, config):
    ...
    # frame_out, vel_out, _ = sample_func(trainer, audio, ema, config)
    ...
    for n in range(audio.shape[0]):
        ...
        # metrics = evaluate(sample, label.reshape(*shape)[n], vel_out[n], vel, band_eval=False)
...
def test_step(trainer, batch, ema, step, device, config):
    ...
    # for seg in tqdm(range(2)):
        ...
        if end > n_step:
            ...
            audio_pad = audio[:, int(start*512):int(end*512)].to(device)
...
def train(rank, world_size, config, ddp=True):
    ...
    if not config.finetune:
        ...
        print(colored("Finetuning pretrained model", "red", attrs=["bold"]))
    ...
    if rank == 0:
        if config.resume_dir:
            run = wandb.init('transcription', name=f'resume_{time}', id=config.id, dir=config.logdir)
        else:   
            run = wandb.init('transcription', config=config, id=config.id, name=f'{config.name}_{time}', dir=config.logdir)
        summary(trainer)
    # configure for ddp
    if ddp:
            ...
            for batch in data_loader_train:
                ...
                if step % config.valid_interval == 0 or step == 5000:
                    ...
                    if rank == 0:
                        ...
                        model_saver.update(trainer, ema, optimizer, scheduler, step, valid_mean['metric/note-with-offsets/f1'], ddp=ddp)
                    if ddp:
                        dist.barrier()
            if step > config.iteration:
                break
    ...
    SAVE_PATH = config.logdir / (Path(best_ckp).stem + f'_eval_{config.dataset}')
    ...
    with th.no_grad():
        for batch in tqdm(iterator, total=len(test_set), desc='Test in progress'):
            ...
            '''
            batch_metric, preds, preds_vel = test_step(model, batch, device)
            for k, v in batch_metric.items():
                test_metrics[k].extend(v)
            for n in range(len(preds)):

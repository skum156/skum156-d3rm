...
import torchvision
from .diffusion.misc import instantiate_from_config, format_seconds, get_model_parameters_info
from .diffusion.distributed import reduce_dict
from .diffusion.distributed import is_primary, get_rank
from .diffusion.lr_scheduler import ReduceLROnPlateauWithWarmup, CosineAnnealingLRWithWarmup
...
class Solver(object):
    def __init__(self, config, args, model, dataloader, logger):
        ...
        self.last_epoch = -1
        self.last_iter = -1
        # self.ckpt_dir = os.path.join(args.logdir, 'checkpoint')
        # self.image_dir = os.path.join(args.logdir, 'images')
    ...
    def step(self, batch, phase='train'):
        ...
        for op_sc_n, op_sc in self.optimizer_and_scheduler.items():
            ...
            if phase == 'train':
                if self.args.amp:
                    with autocast():
                        output = self.model(**input) # input['batch']['image'] : (B, C, H, W), input['batch']['text'] : list
                else:
            ...
            if phase == 'train':
                if op_sc['optimizer']['step_iteration'] > 0 and (self.last_iter + 1) % op_sc['optimizer']['step_iteration'] == 0:
                    op_sc['optimizer']['module'].zero_grad() # AdamW
                    if self.args.amp:
    ...
    def train_epoch(self):
        ...
        for itr, batch in enumerate(self.dataloader['train_loader']):
            # batch['text] : text ifself in list
            # batch['image'] : tensor, [B, C(=3), H, W]
            if itr == 0:
                print("time2 is " + str(time.time()))
            data_time = time.time() - itr_start
            step_start = time.time()
            self.last_iter += 1
            loss = self.step(batch, phase='train') ##### main
